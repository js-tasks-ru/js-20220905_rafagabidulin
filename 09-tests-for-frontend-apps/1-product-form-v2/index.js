...
export default class ProductForm {
  ...
  getProductImagesList() {
    ...
    const items = this.productOptions.images.map(({ url, source }) => this.getImageItem(url, source));

    wrapper.innerHTML = `<ul class="sortable-list">${items}</ul>`;

    const imagesList = wrapper.firstElementChild;

    const sortableList = new SortableList({items});

    imagesList.append(sortableList.element);
  ...
  uploadImage = () => {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      if (file) {
        ...
        this.subElements.imagesList.firstElementChild.append(this.getImageItem(result.data.link, file.name));
  ...
  async render() {
    ...
    [this.categoriesOptions, this.productOptions] = await Promise.all([categoriesPromise, productPromise]);
